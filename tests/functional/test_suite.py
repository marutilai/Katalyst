...
search_read_tests = [
    ...
    KatalystTestCase(
        ...
        task="Find all Python files (.py) in the src/katalyst_agent/nodes directory that import the KatalystState. For each match, show me the line number and the matching line. Then, ask me if I want to see the full content of src/katalyst_agent/nodes/invoke_llm.py.",
...
code_analysis_tests = [
    KatalystTestCase(
        ...
        task="List all function and class definitions in src/katalyst_agent/tools/write_to_file.py. Then, read the content of the write_to_file function itself from that file.",
        expected_output="def write_to_file",
    ),
    KatalystTestCase(
        name="analyze_utils_directory",
        task="Analyze the src/katalyst_agent/utils directory. For each Python file, list its function definitions. Then ask me which function from xml_parser.py I'd like to understand better.",
        expected_output="def",
        auto_approve=False,  # Requires user interaction
    ),
]
...
diff_syntax_tests = [
    KatalystTestCase(
        ...
        task="Read the file src/katalyst_agent/utils/logger.py. Then, propose a diff to change the _LOGGER_NAME from 'katalyst_agent' to 'katalyst_logger'. Apply this diff only after my confirmation. Ensure the syntax is still valid after the change.",
...
complex_tests = [
    ...
    KatalystTestCase(
        ...
        task="Create a new Python file src/katalyst_agent/experiments/sandbox.py. Inside this file, write a simple function called greet(name: str) -> str that returns f'Hello, {name}!'. After writing the file, execute it with the command python src/katalyst_agent/experiments/sandbox.py if it had a main block to print a greeting (modify it to do so if needed, then execute).",
        expected_files={"src/katalyst_agent/experiments/sandbox.py": "def greet"},
    ),
]
