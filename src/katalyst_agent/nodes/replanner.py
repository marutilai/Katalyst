...
def replanner(state: KatalystState) -> KatalystState:
    ...
    """
    logger = get_logger()
    logger.debug(f"[REPLANNER] Starting replanner node. Current task_idx: {state.task_idx}, task_queue length: {len(state.task_queue)}")
    ...
    if state.response:
        logger.debug(f"[REPLANNER] Final response already set in state: '{state.response}'. No replanning needed. Routing to END.")
    ...
    completed_tasks_str = "\n".join(
        f"- '{task_desc}': {summary}" for task_desc, summary in state.completed_tasks
    ) if state.completed_tasks else "No sub-tasks have been completed yet."
    ...
    logger.debug(f"[REPLANNER] Raw LLM response from instructor: {llm_response_model}")
    
    new_subtasks = llm_response_model.subtasks
    logger.info(f"[REPLANNER] LLM proposed new subtasks: {new_subtasks if new_subtasks else 'None (goal likely complete)'}")

    if not new_subtasks:  # LLM returns an empty list, signaling overall task completion
        logger.info("[REPLANNER] LLM indicated original goal is complete (returned empty subtask list).")
        ...
        if state.completed_tasks:
            final_summary_of_work = "Katalyst has completed the following sub-tasks based on the plan:\n" + \
                                    "\n".join([f"- '{desc}': {summ}" for desc, summ in state.completed_tasks]) + \
        ...
        state.chat_history.append(AIMessage(content=f"[REPLANNER] Goal achieved. Final response: {state.response}"))
        logger.debug(f"[REPLANNER] Goal achieved. Setting final response. Task queue empty.")

    else: # LLM provided new subtasks
        logger.info(f"[REPLANNER] Generated new plan with {len(new_subtasks)} subtasks.")
        state.task_queue = new_subtasks
        state.task_idx = 0   # Reset task index for the new plan
        state.response = None # Clear any previous overall response, as we have a new plan
        ...
        logger.debug(f"[REPLANNER] New plan set. Task queue size: {len(state.task_queue)}, Task index: {state.task_idx}")

    logger.debug(f"[REPLANNER] End of replanner node. state.response: '{state.response}', task_queue: {state.task_queue}")
    return state