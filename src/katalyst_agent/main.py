...
def main():
    ...
    final_parsed_call = final_state.get('parsed_tool_call')
    final_iteration = final_state.get('current_iteration', 0)
    max_iter = final_state.get('max_iterations', 10)
    ...
    if final_parsed_call and final_parsed_call.get('tool_name') == 'attempt_completion':
        result = final_parsed_call.get('args', {}).get('result', 'Task successfully completed (no specific result message provided).')
    ...
    # Case 2: Max iterations reached
        ...
        last_llm_response = final_state.get('llm_response_content')
