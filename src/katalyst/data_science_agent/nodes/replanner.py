...
def replanner(state: KatalystState) -> KatalystState:
    ...
    if not state.checkpointer:
        logger.error("[DS_REPLANNER] No checkpointer found in state")
        ...
        if structured_response and isinstance(structured_response, ReplannerOutput):
            if structured_response.is_complete:
                ...
                if structured_response.subtasks:
                    ...
                    logger.info(f"[DS_REPLANNER] Creating new analysis plan with {len(structured_response.subtasks)} tasks")
                    ...
                    # Log new plan
                    plan_message = "Continuing with additional investigations:\n" + "\n".join(
                        f"{i+1}. {task}" for i, task in enumerate(structured_response.subtasks)
                    )
                    logger.info(f"[DS_REPLANNER] {plan_message}")
                else:
                    logger.error("[DS_REPLANNER] Structured response indicates more work needed but no subtasks provided")
                    state.error_message = "Replanner indicated more analysis needed but provided no tasks"
                    state.response = "Unable to determine next analysis steps."
        else:
            # Fallback: check if there's an error message in the result
            logger.error(f"[DS_REPLANNER] No structured response received. Result keys: {list(result.keys())}")
            ...
            ai_messages = [msg for msg in state.messages if isinstance(msg, AIMessage)]
            if ai_messages:
                logger.debug(f"[DS_REPLANNER] Last AI message: {ai_messages[-1].content[:200]}...")
