...
class InputHandler:
    ...
    def prompt_menu(
        ...
        for idx, option in enumerate(options, 1):
            ...
            if show_numbers:
                ...
                table.add_row(
                    number, label, f"[dim]{description}[/dim]" if description else ""
                )
            else:
        ...
        if allow_custom:
            ...
            if show_numbers:
                table.add_row(
                    f"[bold cyan]{custom_idx}.[/bold cyan]",
                    "[italic]Enter custom value[/italic]",
                )
            else:
    ...
    def confirm(
        self, prompt: str, default: bool = True, show_default: bool = True
    ) -> bool:
        """
        ...
        """
        return Confirm.ask(
            prompt, default=default, show_default=show_default, console=self.console
        )
    ...
    def prompt_file_approval(
        ...
        if show_diff and old_content is not None:
            ...
            if len(old_lines) != len(new_lines):
                self.console.print(
                    f"[dim]Line count: {len(old_lines)} → {len(new_lines)}[/dim]"
                )
    ...
    def prompt_arrow_menu(
        ...
        try:
            ...
            self.console.print(
                "[yellow]Arrow key navigation not available. Using numbered menu.[/yellow]"
            )
        ...
        if not (hasattr(sys.stdin, "isatty") and sys.stdin.isatty()):
            # Not in an interactive terminal, use numbered menu
            self.console.print(
                "[yellow]Not in interactive terminal. Using numbered menu.[/yellow]"
            )
            return self.prompt_menu(title, options, show_numbers=True)
        ...
        try:
            ...
            terminal_menu = TerminalMenu(
                ...
                cursor_index=0,  # Start at first option
            )
